...
window.onload = function () {
	...
	function clickHendler(e) {
		let target = e.target;
		headerMenu(target);
		searcherMobile(target)
		searcherSelector(target);
		searcherCategories(target);
		qtySelector(target);
		filterSelector(target);
		removeTag(target);
		cartPreview(target);
	}
}
...
function headerMenu(target) {
	if (window.innerWidth > 768 && isMobile.any()) {
		if (target.classList.contains('menu__arrow')) {
			let targetMenuItem = target.closest('.menu__item');
			if (!targetMenuItem.classList.contains('_hover')) {
				const menuItems = document.querySelectorAll('.menu__item');
				for (const menuItem of menuItems) {
					menuItem.classList.remove('_hover');
				}
			}
			targetMenuItem.classList.toggle('_hover');
		}
		if (!target.classList.contains('menu__arrow')) {
			const menuItems = document.querySelectorAll('.menu__item');
...
function searcherMobile(target) {
	let searcher = document.querySelector('.searcher');
	if (searcher) {
		if (target.classList.contains('header-actions__search')) {
			searcher.classList.toggle('_active');
		}
		if (!target.closest('.searcher') && !target.classList.contains('header-actions__search')) {
			searcher.classList.remove('_active');
		}
	}
}

function searcherSelector(target) {
	let searcherSelect = document.querySelector('.searcher__select');
	let searcherCategories = document.querySelector('.searcher__categories');
	if (searcherSelect) {
		if (!searcherSelect.classList.contains('_slide')) {
			if (target.closest('.searcher__select') && !target.closest('.searcher__categories')) {
				_slideToggle(searcherCategories, 500, searcherSelect);
				searcherSelect.classList.toggle('_active');
			}
			if (!target.closest('.searcher__select') && !target.closest('.searcher__categories')) {
				_slideUp(searcherCategories, 500, searcherSelect);
				searcherSelect.classList.remove('_active');
			}
		}
	}
}

function searcherCategories(target) {
	let searcherTitle = document.querySelector('.searcher__title');
	let searcherQty = document.querySelector('.searcher-title__qty');
	const searcherCategories = document.querySelectorAll('.searcher-categories__checkbox._active');
	if (searcherCategories.length > 0) {
		searcherTitle.classList.add('_categories');
		searcherQty.innerHTML = searcherQty.dataset.text + searcherCategories.length;
	} else {
		searcherTitle.classList.remove('_categories');
	}
}

function filterSelector(target) {
	let filterSelect = document.querySelector('.filter-selector');
	let searcherCategories = document.querySelector('.filter-selector-filters');
	if (filterSelect) {
		if (!filterSelect.classList.contains('_slide')) {
			if (target.closest('.filter-selector') && !target.closest('.filter-selector-filters')) {
				_slideToggle(searcherCategories, 500, filterSelect);
				filterSelect.classList.toggle('_active');
			}
			if (!target.closest('.filter-selector') && !target.closest('.filter-selector-filters')) {
				_slideUp(searcherCategories, 500, filterSelect);
				filterSelect.classList.remove('_active');
			}
		}
	}
}
...
function productsBild() {
	let switchItems = document.querySelectorAll('.switch__item');
	if (switchItems) {
		for (const switchItem of switchItems) {
			switchItem.addEventListener('click', function () {
				const bildType = switchItem.querySelector('input').value;
				rebildProducts(bildType);
			});
		}
	}

	function rebildProducts(bildType) {
		const productsContainer = document.querySelector('.items-section');
		const products = document.querySelectorAll('.item');
		if (productsContainer && products) {
			productsContainer.className = `items-section items-section_${bildType}`;
			_renameClasses(products, `items-section__item item item_${bildType}`);
		}
	}
}

function qtySelectors() {
	let qtySelects = document.querySelectorAll('.qty__selector');
	if (qtySelects) {
		for (let index = 0; index < qtySelects.length; index++) {
			const qtySelect = qtySelects[index];
			qtySelect.addEventListener('click', function (e) {
				const target = e.target;
				const qtyItems = qtySelect.querySelector('.qty__items');
				if (!qtySelect.classList.contains('_slide')) {
					if (target.closest('.qty__selector') && !target.closest('.qty__items')) {
						_slideToggle(qtyItems, 500, qtySelect);
						qtySelect.classList.toggle('_active');
					}
					if (!target.closest('.qty__selector') && !target.closest('.qty__items')) {
						_slideUp(qtyItems, 500, qtySelect);
						qtySelect.classList.remove('_active');
					}
				}
				if (target.classList.contains('qty__item')) {
					let qtyInput = qtySelect.previousElementSibling;
					if (qtyInput) {
						qtyInput.value = target.dataset.qty;
					}
					_slideUp(qtyItems, 500, qtySelect);
					qtySelect.classList.remove('_active');
				}
...
function qtySelector(target) {
	const qtySelectors = document.querySelectorAll('.qty__selector');
	if (qtySelectors) {
		for (let index = 0; index < qtySelectors.length; index++) {
			const qtySelector = qtySelectors[index];
			const qtyItems = qtySelector.querySelector('.qty__items');
			if (!target.classList.contains('qty__selector') && !isSliding(qtySelectors)) {
				_slideUp(qtyItems, 500);
				qtySelector.classList.remove('_active');
			}
		}
	}

	function isSliding(objects) {
		for (const object of objects) {
			if (object.classList.contains('_slide')) {
				return true;
			}
		}
		return false;
...
function cartPreview(target) {
	const cartPreview = document.querySelector('.cart-preview');
	if (target.closest('.header-actions__cart')) {
		if (cartPreview) {
			cartPreview.classList.toggle('_active');
		}
	}
	if (target.closest('.cart-preview__close') || target.closest('.cart-preview__btn_continue')) {
		if (cartPreview) {
			cartPreview.classList.remove('_active');
		}
	}
	if (!target.closest('.header-actions__cart') && !target.closest('.cart-preview')) {
		if (cartPreview) {
			cartPreview.classList.remove('_active');
		}
	}
...
let commentsSlider = new Swiper('.comments-slider__body', {
	observer: true,
	observeParents: true,
	loop: true,
	grabCursor: true,
